...
func GetZimbuIndent(lnum)
  ...
  " Also skip over ">>>" and "<<<" lines.
  if thisline !~ '^\s*#' || prevline =~ '^\s*\(<<<\|>>>\)'
    while prevline =~ '^\s*#' || prevline =~ '^\s*\(<<<\|>>>\)'
  ...
  if prevline =~ '^\s*\(IF\|IFNIL\|ELSEIF\|ELSE\|GENERATE_IF\|GENERATE_ELSEIF\|GENERATE_ELSE\|WHILE\|REPEAT\|TRY\|CATCH\|FINALLY\|FOR\|DO\|SWITCH\|CASE\|DEFAULT\|FUNC\|LAMBDA\|VIRTUAL\|ABSTRACT\|DEFINE\|REPLACE\|FINAL\|PROC\|MAIN\|NEW\|ENUM\|CLASS\|INTERFACE\|PIECE\|INCLUDE\|BITS\|MODULE\|SHARED\)\>\|\<LAMBDA \+([^)]*)$'
    let plindent += &sw
  endif
