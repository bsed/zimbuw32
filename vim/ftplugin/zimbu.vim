...
" braces in comments.
let b:match_words = '\%(^\s*\)\@<=\%(MODULE\|CLASS\|INTERFACE\|BITS\|ENUM\|SHARED\|PIECE\|INCLUDE\|HTML\|CSS\|FUNC\|PROC\|IF\|GENERATE_IF\|WHILE\|REPEAT\|WITH\|DO\|FOR\|SWITCH\|TRY\)\>\|\%(^\s*\)\@<=NEW(.*)\%(\s\+@[a-z=]\+\)*\s*$\|{\s*$:\%(^\s*\)\@<=\%(ELSE\|ELSEIF\|GENERATE_ELSE\|GENERATE_ELSEIF\|CATCH\|FINALLY\)\>:\%(^\s*\)\@<=\%(}\|\<UNTIL\>\)'
