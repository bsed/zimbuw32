...
syn keyword zimbuModule  ARG CHECK E GC IO LOG PROTO SYS HTTP ZC ZUT ZWT T TIME THREAD
...
" For ZUT
syn match zimbuHtmlStart "\<HTML [a-zA-Z_]\+([^)]*).*" nextgroup=zimbuHtml skipnl contains=zimbuAttribute,zimbuHtmlKeyword,@zimbuTypeCluster
syn match zimbuHtmlKeyword "HTML" contained
syn region zimbuHtml start="." end="\ze}" contained contains=@HtmlCode,zimbuZutComment

syn match zimbuCssStart "\<CSS\>" nextgroup=zimbuCss skipnl contains=zimbuCssKeyword
syn match zimbuCssKeyword "CSS" contained
syn region zimbuCss start="." end="\ze}" contained contains=@CssCode,zimbuZutComment
syn match zimbuZutComment contained "  #.*"
syn match zimbuZutComment contained "^ \=#.*"

syn region zimbuInHtml matchgroup=zimbuHtmlBrackets start="[[]=\?" end="]" contains=zimbuStatement,@zimbuTypeCluster contained

syn cluster HtmlCode add=zimbuInHtml,zimbuComment
syn cluster htmlPreproc add=zimbuInHtml,zimbuComment
...
hi def link zimbuHtmlKeyword	Statement
